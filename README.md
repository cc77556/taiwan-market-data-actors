...
Clean JSON APIs for Taiwan stock market and government data — institutional flows, financial statements, dividends, margin/short balances, insider filings, procurement tenders, real estate prices and more. 36 Actors on the [Apify platform](https://apify.com/chamarix), all built the same way:
...
| Actor | Data | History |
|---|---|---|
| [taiwan-corporate-action-prices](https://apify.com/chamarix/taiwan-corporate-action-prices) | Every official price discontinuity — capital reductions, par value changes and ETF splits or reverse splits, with the pre-halt close, the exchange's reopening reference price and the adjustment factor between them, plus the exchange ratio and any cash returned; the published limits and reference price are both re-derived and checked on every row (減資／面額變更／ETF分割參考價) | 2011-01+ |
...
| [taiwan-intraday-5sec-stats](https://apify.com/chamarix/taiwan-intraday-5sec-stats) | The whole market every five seconds — cumulative bid and ask orders and size, matched trade count, volume and turnover, each with its increment, joined on the same timestamp to the TAIEX and 37 more TWSE indices; 3,241 rows a session, with the sampling interval, the closing index and the day's change all reconciled against the exchange's own reports (每5秒委託成交與指數統計) | 2004-10+ |
| [taiwan-ipo-subscription](https://apify.com/chamarix/taiwan-ipo-subscription) | Every IPO ballot and every competitive auction — draw date, subscription window, offer price, what one application costs, valid applications and the allocation odds, beside the auction that set the price: reserve, bids, bid-to-cover and the weighted average award, with the retail discount measurable against it (公開申購抽籤／競價拍賣) | 2006-10+ |
